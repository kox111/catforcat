...
export default function AnalysisModal({
  ...
  useEffect(() => {
    async function analyze() {
      ...
      const totalWords = segments.reduce((sum, s) => sum + wordCount(s.sourceText), 0);
      ...
      const internalRepetitions = segments.filter(
        (s) => (sourceMap.get(s.sourceText) || 0) > 1
      ).length - sourceMap.size + Array.from(sourceMap.values()).filter((v) => v > 1).length;
      ...
      const draft = segments.filter((s) => s.status === "draft" || (s.targetText.trim() !== "" && s.status !== "confirmed")).length;
      const empty = segments.filter((s) => s.targetText.trim() === "" && s.status !== "confirmed").length;
  ...
  return (
    ...
    >
      ...
      >
        ...
        {data.loading ? (
          <div style={{ paddingLeft: "1rem", paddingRight: "1rem", paddingTop: "2rem", paddingBottom: "2rem", textAlign: "center", fontSize: "0.75rem", color: "var(--text-muted)" }}>
            Analyzing project...
          </div>
        ) : (
          <div style={{ paddingLeft: "1rem", paddingRight: "1rem", paddingTop: "1rem", paddingBottom: "1rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
            {/* Overview */}
            ...
            <div>
              <div style={{ fontSize: "0.75rem", fontWeight: 500, marginBottom: "0.5rem", color: "var(--text-secondary)" }}>
                TM Match Distribution
              </div>
              {/* Visual bar */}
              <div style={{ display: "flex", borderRadius: "0.25rem", overflow: "hidden", height: "1rem", marginBottom: "0.5rem" }}>
                {buckets.map((b, i) => {
                  const pct = data.totalSegments > 0 ? (b.count / data.totalSegments) * 100 : 0;
                  if (pct === 0) return null;
                  return (
                    <div
                      key={i}
                      style={{ width: `${pct}%`, background: b.color, minWidth: pct > 0 ? "4px" : "0" }}
              ...
              {/* Legend */}
              {buckets.map((b, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.75rem", paddingTop: "0.125rem", paddingBottom: "0.125rem" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
                    <div style={{ width: "0.625rem", height: "0.625rem", borderRadius: "0.125rem", background: b.color }} />
                    <span style={{ color: "var(--text-secondary)" }}>{b.label}</span>
                  </div>
                  <span style={{ color: "var(--text-primary)" }}>
                    {b.count} ({data.totalSegments > 0 ? Math.round((b.count / data.totalSegments) * 100) : 0}%)
                  </span>
                </div>
              ))}
            </div>
            ...
            {/* Internal Repetitions */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: "0.75rem", paddingTop: "0.5rem", paddingBottom: "0.5rem", borderTop: "1px solid var(--border)" }}>
              <span style={{ color: "var(--text-secondary)" }}>Internal Repetitions</span>
              <span style={{ color: "var(--text-primary)" }}>
                {data.internalRepetitions} segment{data.internalRepetitions !== 1 ? "s" : ""}
              </span>
            </div>
...
function StatBox({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    ...
    >
      <div style={{ fontSize: "1.125rem", fontWeight: 600, color: color || "var(--text-primary)" }}>
