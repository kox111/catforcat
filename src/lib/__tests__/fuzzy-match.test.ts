...
console.log("identical:", levenshteinDistance("hello", "hello") === 0 ? "PASS" : "FAIL");
console.log("one char:", levenshteinDistance("hello", "hallo") === 1 ? "PASS" : "FAIL");
console.log("empty:", levenshteinDistance("", "hello") === 5 ? "PASS" : "FAIL");
console.log("case:", levenshteinDistance("Hello", "hello") === 1 ? "PASS" : "FAIL");
...
const tmEntries: TMEntry[] = [
  {
    id: "1", sourceText: "The cat is on the table",
    targetText: "El gato está en la mesa", srcLang: "en", tgtLang: "es",
    usageCount: 3, createdAt: new Date(),
  },
  {
    id: "2", sourceText: "The cat is on the chair",
    targetText: "El gato está en la silla", srcLang: "en", tgtLang: "es",
    usageCount: 1, createdAt: new Date(),
  },
  {
    id: "3", sourceText: "The dog is in the garden",
    targetText: "El perro está en el jardín", srcLang: "en", tgtLang: "es",
    usageCount: 2, createdAt: new Date(),
  },
  {
    id: "4", sourceText: "Something completely different and much longer text here",
    targetText: "Algo completamente diferente", srcLang: "en", tgtLang: "es",
    usageCount: 1, createdAt: new Date(),
  },
];
...
console.log("Exact match:", exact[0]?.score === 100 ? "PASS" : `FAIL (${exact[0]?.score})`);
...
console.log("Similar match score:", tableMatch?.score,
  (tableMatch?.score && tableMatch.score >= 70 && tableMatch.score <= 90) ? "PASS (70-90%)" : "FAIL");

// Test 3: Top result for similar query should be exact match (chair)
console.log("Top result is exact:", similar[0]?.score === 100 ? "PASS" : `FAIL (${similar[0]?.score})`);
...
console.log("Dog exact match:", dogExact?.score === 100 ? "PASS" : `FAIL (${dogExact?.score})`);
...
console.log("No high match:", noMatch.length === 0 ? "PASS" : `FAIL (${noMatch.length} matches)`);
...
console.log("Length filter:", hasLong === undefined ? "PASS" : "FAIL (long entry included)");

// Test 7: Threshold filtering
const highThreshold = fuzzyMatch("The cat is on the table", tmEntries, 90);
console.log("High threshold filters partials:",
